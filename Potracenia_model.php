...
class Potracenia_model extends CI_Model
{
    var $table = 'pracownik_potracenia';
    ...
    var $order = array('kiedy' => 'asc'); // default order
    ...
    protected function potracenieExists($d, $k, $p)
    {

        try {
            $this->db->trans_begin();
            $this->db->where('pracownik_potracenia.kiedy', $d);
            $this->db->where('pracownik_potracenia.kwota', $k);
            $this->db->where('pracownik_potracenia.fk_pracownik', $p);
    ...
    public function DodajPotracenie($id)
    {
        if (!$this->input->is_ajax_request()) {
            exit('No direct script access allowed');
        }

        $opis = $this->input->post("cf_potracenie_opis");
        $data = $this->input->post("cf_potracenie_data");
        $pracownik = $this->input->post("fk_prac");

        $kwota = $this->custom_decimal($this->input->post('cf_potracenie_kwota'));
        ...
        if ($this->potracenieExists($data, $kwota, $pracownik)) {
        ...
        try {
            ...
            if (empty($message)) {
                ...
                $this->db->insert('pracownik_potracenia', $post_data);
                $status = 1;
